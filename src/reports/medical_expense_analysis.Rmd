...
Regarding to the lack of enough data, the lack of relevant features and the fact that the model is not tuned completely, we think that our model could still be improved. Considering the limited time available for the project, we have not done thorough feature engineering, feature selection, model selection and hyper-parameter tuning. Those last points could be a good starting point for those who would like to improve our model. Overall, this serves as a very good base model on which further improvements can be made.
